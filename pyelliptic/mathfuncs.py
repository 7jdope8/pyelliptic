...
class Normal:
  def __init__(self,val):
    if isinstance(val,Normal): self.val = val.val
    else: self.val = val
...
class ModularInt:
  def __init__(self,val):
    if isinstance(val,ModularInt): self.val = val.val
    else: self.val = val % modulus
...
class Galois:
  def __init__(self,val):
    if isinstance(val,Galois): self.val = val.val
    elif isinstance(val,list): self.val = val
    else:
      self.val = []
      for i in range(32):
